...
Next, we built a predictive model using properties that actually sold in 2024 as training data. We then applied the model to properties that did not sell in 2024, in order to obtain an estimate of what those properties would have sold for. 
...
```
We then performed correlation analysis and best subsets regression to identify which property features are most predictive of sale price.

# model selection
```{r}
# make correlation matrix
cor1 <- cor(train_data[, c('Sale_2024', 'Pre.Yr.Assessed.Total', 'Land.Acres', 'Living.Area', 'Effective.Area', 'Total.Rooms', 'Number.of.Bedroom', 'Number.of.Baths', 'Number.of.Half.Baths', 'ayb', 'eyb')], use = "pairwise.complete.obs")
...
BIC-based stepwise selection chose a model with 3 predictors: Sale Price, eyb, and Total Rooms, yielding an  Adjusted R^2 of about 0.90. Sale Price had the lowest p value, and eyb and Total Rooms also had positive coefficients and significant p values. 

AIC-based stepwise selection chose a model with 6 predictors: Effective Area, Total Rooms, Number of Bedrooms, Sale Price, ayb, and eyb. All coefficients except Number of Bedrooms are positive, and the Adjusted R^2 is about 0.90. 

We chose the AIC-based model as our final model, as it had a slightly higher Adjusted R^2 and included more predictors. All coeffients in our model are positive, and all predictors are significnat. Number of Baths and Number of Bedrooms have the largest coefficients, while Land Acres has the smallest. 
...
ggplot(data = train_data, aes(x = `Current Year Appraisal.y`, y = Sale_2024)) + geom_point() + geom_abline(slope = 1, intercept = 0, linetype = "dashed", color = "red")
